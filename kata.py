...
class KatamariEditor:
    ...
    def build_mesh_generator(self, parent):
        ...
        # Export button
        tk.Button(mesh_frame, text="Export Mesh as OBJ", command=self.export_mesh_obj,
    ...
    def export_mesh_obj(self):
        """Export generated mesh as OBJ file"""
        if not self.generated_mesh:
            messagebox.showwarning("Error", "No mesh to export. Generate a mesh first.")
            return
        ...
        try:
            with open(filepath, 'w') as f:
                f.write("# Generated by Katamari Entity Editor\n")
                f.write(f"# Mesh type: {self.generated_mesh['type']}\n\n")

                if self.generated_mesh['type'] == 'floor':
                    X = self.generated_mesh['X']
                    Y = self.generated_mesh['Y']
                    Z = self.generated_mesh['Z']
                    ...
                    # Write faces (quads split into triangles)
                    for i in range(rows - 1):
                        for j in range(cols - 1):
                            # Get corner vertices
                            v1 = vertex_map.get((i, j))
                            v2 = vertex_map.get((i, j+1))
                            v3 = vertex_map.get((i+1, j+1))
                            v4 = vertex_map.get((i+1, j))
            ...
            self.lbl_info.config(text=f"Mesh exported to {filepath}")
            messagebox.showinfo("Success", f"Mesh exported to:\n{filepath}")

        except Exception as e:
            messagebox.showerror("Error", f"Failed to export mesh: {str(e)}")
    ...
    def plot_all(self):
        ...
        if hasattr(self, 'generated_mesh') and self.generated_mesh:
            ...
            if self.generated_mesh['type'] == 'floor':
                ...
                self.ax3d.plot_surface(-X_mesh, Z_mesh, Y_mesh, color='green', alpha=mesh_alpha,
                                       shade=True, edgecolor='darkgreen', linewidth=0.3)
                ...
                mesh_poly = Poly3DCollection(verts, alpha=mesh_alpha, facecolors='green',
